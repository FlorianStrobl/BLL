...
(function (Lexer) {
  ...
  const maxSymbolLen = symbols.reduce(
  ...
  function consumeSymbol(code, idx) {
    ...
    let potentialSymEndIdx = Math.min(
...
(function (Parser) {
  ...
  function parseArgumentList(
    parseArgument, // assertion: does not have delimiter as valid value
    endToken,
    delimiter,
    commentInfo,
    emptyList,
  ...
  function parseComplexeTypeLine() {
    ...
    const identifierToken = matchTypeAdvanceOrError(
    ...
    const closingBracketToken = callOnPresent(openingBracketToken, () =>
      matchAdvOrErr(
        ')',
  ...
  function parseTypeExprLv1() {
    ...
    while (match(['.', '['])) {
      ...
      if (match('.')) {
        ...
        consComments(comments);
        checkEofErr('Eof while parsing a type-expression.');
        const propertyToken = matchTypeAdvanceOrError(
          Parser.tokenType.identifier,
        ...
        consComments(comments);
        checkEofErr('Eof while parsing a type-expression.');
        const substitutions = parseArgumentList(
          parseTypeExpression,
          ']',
          {
            delimiterToken: ',',
        ...
        const closingBracketToken = matchAdvOrErr(
          ']',
  ...
  function parseFuncExprParams(lastHadDefaultVal, comments) {
    ...
    consComments(comments);
    checkEofErr('Eof while parsing arguments of a function expression.');
    const typeExpression = callOnPresent(colonToken, () =>
    ...
    const value = callOnPresent(defaultValEqToken, () =>
  ...
  function parseFuncExpression() {
    ...
    const openingBracketToken = matchAdvOrErr(
      '(',
    ...
    const params = parseArgumentList(
    ...
    consComments(comments);
    checkEofErr('Eof while parsing a function expression.');
    const arrowToken = matchAdvOrErr(
      '=>',
  ...
  function parseMatchBodyLine(hadDefaultLine) {
    ...
    consComments(comments);
    checkEofErr('Eof while parsing a match body line expression.');
    const openingBracketToken = callOnPresent(identifierToken, () =>
      optMatchAdvance('(')
    );
    consComments(comments);
    checkEofErr('Eof while parsing a match body line expression.');
    const params = callOnPresent(identifierToken, () =>
      callOnPresent(openingBracketToken, () =>
        parseArgumentList(
          () =>
    ...
    const closingBracketToken = callOnPresent(identifierToken, () =>
      callOnPresent(openingBracketToken, () =>
  ...
  function parseMatchExpression() {
    ...
    const argOpeningBracketToken = matchAdvOrErr(
      '(',
      'Missing "(" token in match expression.'
    );
    consComments(comments);
    ...
    consComments(comments);
    checkEofErr('Eof while parsing a match expression.');
    const argClosingBracketToken = matchAdvOrErr(
      ')',
    ...
    consComments(comments);
    checkEofErr('Eof while parsing a match expression.');
    const typeAnnotation = callOnPresent(colonToken, () =>
      !match('{') // better error message
        ? parseTypeExpression()
        : newParseError(
    ...
    consComments(comments);
    checkEofErr('Eof while parsing a match expression.');
    const bodyOpeningBracketToken = matchAdvOrErr(
      '{',
      'Missing "{" token in match expression.'
    );
    ...
    consComments(comments);
    checkEofErr('Eof while parsing a match expression.');
    const bodyClosingBracketToken = matchAdvOrErr(
      '}',
      'Missing "}" token in match expression.'
    );
    const explicitType =
  ...
  function parseStatement() {
    ...
    if (match('let')) {
      ...
      const genericIdentifiers = callOnPresent(genericOpeningBracketToken, () =>
        parseArgumentList(
          () =>
            matchTypeAdvanceOrError(
              Parser.tokenType.identifier,
      ...
      const genericClosingBracketToken = callOnPresent(
      ...
      const equalsToken = matchAdvOrErr(
        '=',
      ...
      consComments(comments);
      checkEofErr('Eof in let-statement.');
      const semicolonToken = matchAdvOrErr(
        ';',
    ...
    if (match('use')) {
      ...
      const filename = matchTypeAdvanceOrError(
        Parser.tokenType.identifier,
...
(function (Interpreter) {
  ...
  function deepCpy(value) {
    ...
    switch (typeof value) {
      case 'object':
        if (Array.isArray(value)) return value.map((val) => deepCpy(val));
        const newObj = {};
  ...
  function executeExpr(expr, lets, types) {
    ...
    switch (parseExpr.type) {
      ...
      case 'match':
        ...
        const defaultLineIdx = parseExpr.body.findIndex(
...
(function (Formatter) {
  ...
  function printComments(comments, indentation) {
    return comments.length === 0
      ...
      : comments
          .map(
            (comment) =>
              indentation + addColor(comment.l, Formatter.Colors.comments)
          )
          .join('\n') + '\n';
  }
          ...
          ? addColor('[', Formatter.Colors.symbol) +
            statement.genericIdentifiers
              .map((genericIdentifier) =>
                addColor(
                  genericIdentifier.argument.l,
                  Formatter.Colors.genericIdentifier
                )
              )
              .join(addColor(', ', Formatter.Colors.symbol)) +
            addColor(']', Formatter.Colors.symbol)
  ...
  function beautify(
    code,
    settings = { withColor: true, withComments: true, defaultIndentation: '  ' }
  ) {
